///<summary>
///Reporte de comprobantes de egresos
///</summary>
report 88105 "D365L Cheque M"
{
    ApplicationArea = All;
    UsageCategory = ReportsAndAnalysis;
    DefaultLayout = RDLC;
    RDLCLayout = 'Report/D365LChequeM.rdl';
    Caption = 'Cheque M';
    ...
    dataset
    {
        dataitem(VendorLedgerEntry; "Vendor Ledger Entry")
        {
            ...
            column(VendLedEntAmountInWordsDolar; AmountInWordsDolar) { }
            ...
            dataitem(Vendor; Vendor)
            {
                ...
                dataitem(User; User)
                {
                    ...
                    {
                        DataItemLinkReference = VendorLedgerEntry;
                        DataItemTableView = sorting("Check No.");
            ...
            dataitem(G_LEntry; "G/L Entry")
            {
                ...
                trigger OnAfterGetRecord()
                begin
                    // Evitar duplicar por cada Vendor Ledger Entry
            ...
            begin
                ...
                if (currencys = '') or (currencys = 'COP') then
                    currency_Txt := 'COP'
                else
                ...
                totalAmount_ := CalculateTotalAmountv2(
                ...
                // ── Obtener TRM vigente a la fecha de registro ────────────────
                TRM := 0;
                CurrExchRate.Reset();
                CurrExchRate.SetRange("Currency Code", 'USD');
                CurrExchRate.SetFilter("Starting Date", '<=%1', VendorLedgerEntry."Posting Date");
                if CurrExchRate.FindLast() then begin
                    if CurrExchRate."Exchange Rate Amount" > 0 then
                        // TRM = cuántos COP equivalen a 1 USD
                        TRM := CurrExchRate."Relational Exch. Rate Amount" / CurrExchRate."Exchange Rate Amount";
                end;

                // ── Normalizar amountwtax a COP ───────────────────────────────
                // CalculateTotalAmountv2 puede retornar COP o USD según el doc.
                // Usamos Original Currency Factor para llevar todo a COP (LCY)
                if currency_Txt = 'USD' then begin
                    // amountwtax está en USD → convertir a COP para el campo en pesos
                    currencyF := VendorLedgerEntry."Original Currency Factor";
                    if currencyF > 0 then
                        amountInCOP := Round(amountwtax * currencyF, 1)  // USD * TRM = COP
                    else if TRM > 0 then
                        amountInCOP := Round(amountwtax * TRM, 1)
                    else
                        amountInCOP := amountwtax;

                    // USD ya lo tenemos directo
                    amountInUSD := amountwtax;
                end else begin
                    // amountwtax ya está en COP
                    amountInCOP := amountwtax;
                    currencyF := VendorLedgerEntry."Original Currency Factor";
                    if VendorLedgerEntry."Original Currency Factor" > 0 then
                        currencyF := 1 / VendorLedgerEntry."Original Currency Factor";

                    // Convertir COP → USD usando TRM
                    if TRM > 0 then
                        amountInUSD := Round(amountInCOP / TRM, 0.01)
                    else if currencyF > 0 then
                        amountInUSD := Round(amountInCOP * currencyF, 0.01)
                    else
                        amountInUSD := 0;
                end;

                // ── PESOS (COP) ───────────────────────────────────────────────
                numberToText.FormatNoText(AmountText, amountInCOP, 'COP');
                ...
                // ── DOLARES (USD) ─────────────────────────────────────────────
                numberToText.FormatNoText(AmountTextUSD, amountInUSD, 'USD');
                AmountInWordsDolar := AmountTextUSD[1] + AmountTextUSD[2];

                if not AmountInWordsDolar.Contains(' DOLARES ') then
                    AmountInWordsDolar := AmountTextUSD[1] + ' DOLARES ' + AmountTextUSD[2];
            end;
    ...
    local procedure CalculateTotalAmountv2(DocumentNo: Code[20]; VLEN: Integer; postingDate: Date): Decimal
    var
        total: Decimal;
    ...
    begin
        ...
        if VendorLedgerEntry.FindSet() then
            repeat
                ...
                DetailedVendorLedgEntry.setfilter(Unapplied, '%1', false);
                DetailedVendorLedgEntry.CalcSums(Amount);
                total += DetailedVendorLedgEntry.Amount;
            until VendorLedgerEntry.Next() = 0;

        // si no tiene documentos liquidados busca en mov bancarios
        if total = 0 then begin
            BankAccountLedgerEntry.SetFilter("Document No.", DocumentNo);
            BankAccountLedgerEntry.SetFilter("Posting Date", '%1', postingDate);
            if BankAccountLedgerEntry.CalcSums(Amount) then
